...
def _make_stage(raw_dim: int, target_dim: int, output_dim: int, tensor_count: int) -> tuple[StageRuntime, int]:
    ...
    model = HTFRModel.from_tensors(tensors, top_k=min(2, tensor_count), randomize_interpolations=False)
    return StageRuntime(projector=projector, model=model, loss="mse" if output_dim != 3 else "logits_ce"), output_dim


def test_hyperfield_transformer_step_and_diagnostics() -> None:
    ...
    stage2_model = HTFRModel.from_tensors(stage2_tensors, top_k=2, randomize_interpolations=False)
    stage2 = StageRuntime(projector=stage2_projector, model=stage2_model, loss="logits_ce")

    hft = HyperFieldTransformer(stage1, stage2, tail_token_count=2, tail_embedding_dim=1)
    ...
    eval_logits, _ = hft.step(context, tail_embeddings, target_token=None, stage1_target=None, train=False)
    assert eval_logits.shape == (3,)

    diag = hft.diagnostics()
